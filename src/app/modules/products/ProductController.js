...
const createProductCategory = async (req, res) => {
  ...
  if (!name.length > 0) {
    return res
      .status(400)
      .json({ success: false, message: "Please provide all mandatory fields" });
  }
  try {
    // Insert new product
    body.totalCount = 0;
    const newProductCategory =
      await Collections.productCategoriesCollection.insertOne(body);
    res
      .status(201)
      .json({ success: true, message: "New product category uploaded!" });
  } catch (error) {
...
const createProduct = async (req, res) => {
  ...
  try {
    ...
    const productCategoryToUpdate =
      await Collections.productCategoriesCollection.findOne({ name: category });
    ...
    res
      .status(201)
      .json({ success: true, message: "New product uploaded!", productId });
  } catch (error) {
    console.error("Error creating product:", error);
    res.status(500).json({ success: false, message: "Internal server error" });
  }
...
const deleteProduct = async (req, res) => {
  ...
  const productCategoryToUpdate =
    await Collections.productCategoriesCollection.findOne({
